...
export default function AuraApp() {
  ...
  const handleProcessSale = async (cartItems, total, totalProfit) => {
    if (!user) return;
    ...
    try {
      ...
      await batch.commit();
      setCart([]);
      showNotification(`Venta registrada. Ganancia: Q${totalProfit.toFixed(2)}`);
    } catch (error) {
        ...
        <header className="bg-white p-6 border-b border-pink-50 shadow-sm flex justify-between items-center sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
             <span className="text-sm font-medium text-slate-500">
               {user ? 'Usuario Conectado' : 'Conectando...'}
             </span>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <StatCard title="Valor Inventario (Costo)" value={`Q${totalStockValue.toFixed(2)}`} icon={<Package className="text-purple-500" />} color="bg-purple-50" />
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-6 border-b border-slate-50">
          <h3 className="font-semibold text-lg text-slate-700">Últimas Ventas</h3>
        </div>
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 text-slate-500 uppercase font-medium text-xs">
              <tr>
                ...
                <th className="px-6 py-4 text-emerald-600">Ganancia</th>
              </tr>
            </thead>
              ...
              {sales.length === 0 ? (
                ...
                sales.slice(0, 10).map((sale) => (
                  <tr key={sale.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 font-medium">#{sale.id.slice(0, 8)}</td>
                    <td className="px-6 py-4">{new Date(sale.date).toLocaleString()}</td>
                    <td className="px-6 py-4">{sale.items.reduce((a, b) => a + b.quantity, 0)} productos</td>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-pink-100 sticky top-24">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              ...
              <input 
                type="text" required
                className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-400"
                value={formData.name}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-400 bg-slate-50"
                  value={formData.cost}
            ...
            <div className="bg-pink-50 p-4 rounded-lg border border-pink-100">
              ...
              <input 
                type="number" step="0.01" min="0" required
                className="w-full px-4 py-2 border border-pink-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-400 font-bold text-slate-800"
            ...
            <div className="flex gap-2 pt-2">
              ...
              {isEditing && (
                <button type="button" onClick={() => { setIsEditing(false); setFormData(initialFormState); }} className="px-4 py-2 bg-slate-200 text-slate-600 rounded-lg hover:bg-slate-300">
                  Cancelar
                </button>
              )}
            </div>
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          {filteredProducts.length === 0 && <div className="p-8 text-center text-slate-400">Sin productos.</div>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex h-[calc(100vh-140px)] gap-6">
        ...
        <div className="flex-1 overflow-y-auto pr-2 grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 content-start">
          {filteredProducts.map(product => (
            <button
              ...
              className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 hover:border-pink-300 hover:shadow-md transition-all text-left flex flex-col justify-between group h-32"
            >
              <div>
      ...
      <div className="w-96 bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col h-full">
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 rounded-b-xl space-y-4">
          ...
          <button 
            onClick={() => onCheckout(cart, cartTotal, cartProfit)}
